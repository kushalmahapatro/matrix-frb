...
fn main() {
    ...
    // Linux prebuilt runs llvm-objcopy to prefix BoringSSL symbols; Android upstream script does not,
    // so linking SQLCipher (OpenSSL) + libwebrtc causes duplicate ASN1_* / BN_* / etc.
    if target_os == "android" {
        android_prefix_boringssl_in_libwebrtc(&webrtc_lib);
    }
...
/// Match `build_linux.sh`: prefix BoringSSL symbols in the static archive so OpenSSL (e.g. SQLCipher)
/// can link into the same `cdylib` without duplicate symbol errors.
fn android_prefix_boringssl_in_libwebrtc(webrtc_lib: &Path) {
    let lib_a = webrtc_lib.join("libwebrtc.a");
    let pristine = webrtc_lib.join("libwebrtc.pristine.a");
    let stamp = webrtc_lib.join(".libwebrtc_boringssl_prefixed");
    let manifest_dir = PathBuf::from(env::var("CARGO_MANIFEST_DIR").unwrap());
    let sym_file = manifest_dir.join("libwebrtc/boringssl_prefix_symbols.txt");

    println!("cargo:rerun-if-changed={}", sym_file.display());

    if !lib_a.is_file() || !sym_file.is_file() {
        return;
    }

    let sym_mtime = fs::metadata(&sym_file).ok().and_then(|m| m.modified().ok());
    let stamp_mtime = fs::metadata(&stamp).ok().and_then(|m| m.modified().ok());
    let need_run = !stamp.is_file()
        || sym_mtime
            .zip(stamp_mtime)
            .map(|(s, t)| s > t)
            .unwrap_or(true);
    ...
    let toolchain = webrtc_sys_build::android_ndk_toolchain()
        .expect("ANDROID_NDK_HOME required to prefix BoringSSL in libwebrtc.a");
    let mut objcopy = toolchain.join("bin/llvm-objcopy");
    if cfg!(target_os = "windows") {
        objcopy.set_extension("exe");
    }

    if pristine.is_file() {
        fs::copy(&pristine, &lib_a).unwrap_or_else(|e| {
            panic!(
                "restore libwebrtc.a from {}: {e}",
                pristine.display()
            )
        });
    } else {
        fs::copy(&lib_a, &pristine).unwrap_or_else(|e| {
            panic!(
                "snapshot pristine libwebrtc.a to {}: {e}",
                pristine.display()
            )
        });
    }
    ...
    let status = Command::new(&objcopy)
        .args(["--redefine-syms", sym_file.to_str().unwrap()])
        .arg(&lib_a)
        .status()
        .unwrap_or_else(|e| panic!("failed to run {:?}: {e}", objcopy));

    if !status.success() {
        panic!(
            "llvm-objcopy --redefine-syms failed on {} (exit {:?})",
            lib_a.display(),
            status.code()
        );
    }

    fs::write(&stamp, b"1").expect("write boringssl prefix stamp");
}
