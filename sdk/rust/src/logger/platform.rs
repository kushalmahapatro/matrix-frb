//! Platform init and tracing for the Flutter/Matrix app.
//!
//! Reuse with parent crate (matrix-sdk-ffi): This module and `crate::api::tracing` are
//! maintained in this repo for the Flutter bridge (GLOBAL_RUNTIME, StreamSink, etc.). The
//! parent crate `matrix-sdk-ffi` has its own `platform` and `platform::tracing` for
//! uniffi (Kotlin/Swift/JS). Sharing one implementation would require either:
//! - A shared crate both depend on (e.g. matrix-sdk-common or a new matrix-sdk-platform),
//!   or
//! - This crate depending on matrix-sdk-ffi and delegating init (would require aligning
//!   runtime handling: we use GLOBAL_RUNTIME; matrix-sdk-ffi uses async_compat).
...
use crate::logger::tracing::LogLevel;

static GLOBAL_INIT_STATE: OnceCell<bool> = OnceCell::new();

fn get_global_init_state() -> bool {
    match GLOBAL_INIT_STATE.get() {
        Some(init_state) => *init_state,
        None => false,
    }
}

fn set_global_init_state(init_state: bool) -> Result<(), String> {
    GLOBAL_INIT_STATE
        .set(init_state)
...
/// Broadcast channel sender for streaming tracing logs to Dart (e.g. iOS simulator console).
static TRACING_BROADCAST_TX: OnceLock<tokio::sync::broadcast::Sender<String>> = OnceLock::new();
...
#[derive(Default)]
pub struct FieldsFormatterForFiles(DefaultFields);
...
/// Writer that buffers the full formatted output for one event and sends it as a single
/// string to the broadcast channel (so Dart gets the exact log string including multi-line
/// backtraces, not split by line).
struct BroadcastLogWriter {
    tx: std::sync::Arc<tokio::sync::broadcast::Sender<String>>,
    buf: String,
}
...
impl Write for BroadcastLogWriter {
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        self.buf
            .push_str(std::str::from_utf8(buf).unwrap_or("\u{fffd}"));
        Ok(buf.len())
    }
...
impl Drop for BroadcastLogWriter {
    fn drop(&mut self) {
        if !self.buf.is_empty() {
            let full = std::mem::take(&mut self.buf);
            let _ = self.tx.send(full.trim_end().to_string());
...
/// MakeWriter that produces BroadcastLogWriter for the tracing broadcast layer.
struct BroadcastMakeWriter(std::sync::Arc<tokio::sync::broadcast::Sender<String>>);
...
{
    #[derive(Default)]
    struct FieldsFormatterBroadcast(DefaultFields);
    impl<'writer> FormatFields<'writer> for FieldsFormatterBroadcast {
        fn format_fields<R: RecordFields>(
            &self,
            writer: Writer<'writer>,
            fields: R,
    ...
    fmt::layer()
        .fmt_fields(FieldsFormatterBroadcast::default())
        .event_format(EventFormatter::new())
        .with_ansi(false)
        .with_writer(BroadcastMakeWriter(std::sync::Arc::new(tx)))
}
...
fn make_file_layer(
    ...
    RollingFileAppender,
> {
    let rotation = match file_configuration.rotation {
        FileRotation::Hourly => Rotation::HOURLY,
        FileRotation::Daily => Rotation::DAILY,
        FileRotation::Weekly => Rotation::WEEKLY,
...
impl TraceLogPacks {
    ...
    fn targets(&self) -> &[LogTarget] {
        match self {
            ...
            TraceLogPacks::MatrixSdkHttp => &[LogTarget::MatrixSdkHttpClient],
        }
    }
}
...
struct SentryLoggingCtx {
    ...
    pub _guard: sentry::ClientInitGuard,
...
struct LoggingCtx {
    ...
    #[cfg(feature = "sentry")]
    pub sentry: Option<SentryLoggingCtx>,
}
...
impl TracingConfiguration {
    ...
    fn build(mut self) -> LoggingCtx {
        ...
        // Channel for streaming tracing logs to Dart (e.g. iOS simulator console).
        let (tracing_tx, _) = tokio::sync::broadcast::channel(256);
        let _ = TRACING_BROADCAST_TX.set(tracing_tx.clone());
        let broadcast_layer = make_broadcast_layer(tracing_tx);
        ...
        // Bridge log crate (e.g. log_panics with backtrace) into tracing so it goes to
        // the same layers (file, broadcast, etc.). Ignore errors if log is already set.
...
fn build_tracing_filter(config: &TracingConfiguration) -> String {
    ...
    // Default for unspecified targets (e.g. Dart log_event): use the chosen log level.
    // trace → all; debug → debug+; info → info+; warn → warn+; error → error only.
    let mut filters = vec![global_level.as_str().to_owned(), "panic=error".to_owned()];
...
pub fn init_platform(
    ...
    let init_state = get_global_init_state();
    if init_state {
        return Ok(());
    }
    ...
    let _ = set_global_init_state(true);

    Ok(())
}

/// Returns the global runtime handle if platform has been initialized (e.g. for use by
/// matrix-sdk-ffi when both crates are in the same process).
/// Not exposed to Dart (Handle cannot be serialized); use from Rust only.
#[cfg(not(target_family = "wasm"))]
#[frb(ignore)]
pub fn get_runtime_handle() -> Option<tokio::runtime::Handle> {
    GLOBAL_RUNTIME.get().map(|r| r.handle().clone())
}
...
/// Target string for Matrix SDK HTTP client tracing. Used to filter HTTP logs.
const MATRIX_SDK_HTTP_CLIENT_TARGET: &str = "matrix_sdk::http_client";
...
/// events.
pub fn subscribe_tracing_logs(sink: StreamSink<String>) {
    let Some(tx) = TRACING_BROADCAST_TX.get() else {
        return;
    };
        ...
        loop {
            match rx.recv().await {
                ...
                Err(tokio::sync::broadcast::error::RecvError::Lagged(n)) => {
                    let _ = sink.add(format!("[tracing] dropped {n} log line(s)"));
                }
...
pub fn subscribe_http_tracing_logs(sink: StreamSink<String>) {
    let Some(tx) = TRACING_BROADCAST_TX.get() else {
        return;
    };
    let mut rx = tx.subscribe();
    ...
    rt.spawn(async move {
        loop {
            match rx.recv().await {
                Ok(line) => {
                    if line.contains(MATRIX_SDK_HTTP_CLIENT_TARGET) {
                        let _ = sink.add(line);
                    }
                }
                Err(tokio::sync::broadcast::error::RecvError::Lagged(n)) => {
                    let _ = sink.add(format!("[http tracing] dropped {n} log line(s)"));
                }
                Err(tokio::sync::broadcast::error::RecvError::Closed) => break,
            }
        }
