use crate::rhttp::api::error::RhttpError;
use crate::rhttp::api::http::HttpVersionPref;
use crate::rhttp::utils::socket_addr::SocketAddrDigester;
...
/// Timeouts in milliseconds. FRB does not support chrono::Duration, so we use i64.
...
static DEFAULT_CLIENT: once_cell::sync::OnceCell<RequestClient> = once_cell::sync::OnceCell::new();

/// Returns a default HTTP client (created with default settings). Use this when calling
/// [make_http_request] or [make_http_request_receive_stream] without a specific client.
#[frb(sync)]
pub fn get_default_client_sync() -> RequestClient {
    DEFAULT_CLIENT.get_or_init(RequestClient::new_default).clone()
}
...
fn create_client(settings: ClientSettings) -> Result<RequestClient, RhttpError> {
    let client: reqwest::Client = {
        ...
        if let Some(timeout_settings) = settings.timeout_settings {
            if let Some(ms) = timeout_settings.timeout_ms.filter(|&m| m > 0) {
                client = client.timeout(StdDuration::from_millis(ms as u64));
            }
            if let Some(ms) = timeout_settings.connect_timeout_ms.filter(|&m| m > 0) {
                client = client.connect_timeout(StdDuration::from_millis(ms as u64));
            }
            if let Some(ms) = timeout_settings.keep_alive_timeout_ms.filter(|&m| m > 0) {
                let d = StdDuration::from_millis(ms as u64);
                client = client.tcp_keepalive(d);
                client = client.http2_keep_alive_while_idle(true);
                client = client.http2_keep_alive_timeout(d);
            }
            if let Some(ms) = timeout_settings.keep_alive_ping_ms.filter(|&m| m > 0) {
                client = client.http2_keep_alive_interval(StdDuration::from_millis(ms as u64));
            }
        }

        if let Some(tls_settings) = settings.tls_settings {
            let certs: Vec<Certificate> = tls_settings
        ...
        client = match settings.http_version_pref {
            ...
            // http3 requires reqwest_unstable; use default for Http3 pref
            HttpVersionPref::Http3 | HttpVersionPref::All => client,
        };
    ...
    // Allow matrix-sdk to use this client via shared_http (set_shared_reqwest_client).
    shared_http::set_shared_reqwest_client(client.clone());
